...
## Шаг 2: Запуск Frontend приложения
...
## Шаг 3: Настройка для реального устройства

⚠️ **Важно**: Если вы запускаете на реальном телефоне, нужно изменить адрес сервера.
...
npm run web      # Только web версия
npm run ios      # Только iOS
npm run android  # Только Android
```
